...
describe.skipIf(!enabled)('the comments domain against the live API', () => {
	const specs = defineCommentAcceptanceSpecs({
		...
		file: enabled ? requireSystemEnv('FIGMA_COMMENT_FILE_KEY') : '',
		includeWrites: Boolean(systemEnv('FIGMA_COMMENT_WRITES')),
	})
	specs()
})
